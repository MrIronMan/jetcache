...
public class CacheTest {
    ...
    public void testTryLock_Null() {
        assertNull(cache.tryLock(null, null, 1, TimeUnit.HOURS));
    }

    @Test
    public void testTryLock_Success() {
        AutoReleaseLock lock = cache.tryLock("key", Integer.class, 1, TimeUnit.HOURS);
        assertNotNull(lock);
        assertNotNull(concreteCache.get("key", Integer.class));
    }
    ...
    public void testTryLock_LockAndGetRetry4() {
        ...
        when(cache.GET(any(), any())).thenReturn(new CacheGetResult(CacheResultCode.FAIL,null,null))
                .then(delegateAnswer);
        AutoReleaseLock lock = cache.tryLock("key", Integer.class, 1, TimeUnit.HOURS);
        assertNotNull(lock);
    }
    ...
    public void testTryLock_Unlock1() {
        AutoReleaseLock lock = cache.tryLock("key", Integer.class, 1, TimeUnit.HOURS);
        lock.close();
        assertEquals(CacheResultCode.NOT_EXISTS, concreteCache.GET("key", Integer.class).getResultCode());
    }
    @Test
    ...
    public void testTryLock_Unlock5() throws Exception {
        ...
        AutoReleaseLock lock = cache.tryLock("key", Integer.class, 1, TimeUnit.MILLISECONDS);
        Thread.sleep(2);
        lock.close();
        assertNotNull(concreteCache.GET("key", Integer.class));
    }
